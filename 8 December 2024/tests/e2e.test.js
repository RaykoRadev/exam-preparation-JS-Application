...
const mockData = {
    ...
    "catalog": [
        ...
        },
        {
            ...
            "phone": "0983244321",
            "description":
                "TEST-The DJI Inspire 3 is a professional-grade aerial platform designed for filmmakers and content creators. Featuring an 8K camera with a large 4/3 CMOS sensor, it delivers breathtakingly high-resolution video and stunning low-light performance. The Inspire 3 boasts advanced AI capabilities and intelligent flight modes.",
...
describe("E2E tests", function () {
    ...
    describe("Authentication [ 20 Points ]", function () {
        ...
        it("Login shows alert on fail and does not redirect [ 2.5 Points ]", async function () {
            ...
            await Promise.all([onResponse(), page.click('[type="submit"]'), Promise.race([alertPromise, errorMessageSpanPromise])]);
    ...
    describe("Home Page [ 5 Points ]", function () {
        it("Show Home page text [ 2.5 Points ]", async function () {
            ...
            await page.waitForSelector(
                "text=Discover the best deals on drones! Buy, sell, and trade top-quality drones with ease on Drone Deals - your trusted marketplace for all things drone.",
                { timeout: interval },
            );
            expect(
    ...
    describe("Dashboard Page [ 15 Points ]", function () {
        ...
        it("Check Drones have correct images [ 2.5 Points ]", async function () {
            ...
            const images = await page.$$eval(".drone img", t => t.map(s => s.src));
        ...
        it("Check Drones have correct model [ 2.5 Points ]", async function () {
            ...
            const categories = await page.$$eval(".drone .model", t => t.map(s => s.textContent));
        ...
        it("Check Drones have correct price [ 2.5 Points ]", async function () {
            ...
            const categories = await page.$$eval(".drone-info .price", t => t.map(s => s.textContent));
        ...
        it("Check Drones have correct condition [ 2.5 Points ]", async function () {
            ...
            const categories = await page.$$eval(".drone-info .condition", t => t.map(s => s.textContent));
        ...
        it("Check Drones have correct type [ 2.5 Points ]", async function () {
            ...
            const categories = await page.$$eval(".drone-info .weight", t => t.map(s => s.textContent));
...
async function handleRaw(match, handlers) {
    ...
    function request(method, returns, options) {
        ...
        return {
            ...
            onResponse: () =>
                context.waitForResponse(response => urlPredicate(response.request()) && response.request().method() === method),
            isHandled: () => handled,
        };
    }
